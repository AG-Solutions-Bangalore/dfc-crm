...
const FittedTyreReport = () => {
  ...
  const getTyrePositions = (vehicleData) => {
    ...
    if (vehicleData.some(item => item.tyre_assign_3_back_housing_left_no || item.tyre_assign_3_back_housing_left_type)) {
      return [
        { 
          name: '1.Front Left', 
  ...
  const handleExcelExport = async () => {
    ...
    try {
          ...
          tyrePositions.forEach((position) => {
            ...
            if (tyreNo || tyreType || tyreMake) {
              worksheet.addRow([
                ...
                km || '',
                preDate && preDate !== '0000-00-00' ? moment(preDate).format('DD-MM-YYYY') : '',
                preKm || '',
                kmRun,
                status || '',
              ])
      ...
      link.href = url
      link.download = `Fitted-Tyre-Report-${moment().format('DD-MM-YYYY')}.xlsx`
      link.click()
                            ...
                            >
                              <div className="p-3 bg-blue-50 font-bold border-b border-gray-300">
                                <div className="flex justify-between">
                                  <span className="text-blue-700">Vehicle No: {vehicleNo}</span>
                                  <span className="text-gray-700">Branch: {firstItem.tyre_assign_branch || 'N/A'}</span>
                                </div>
                              </div>
